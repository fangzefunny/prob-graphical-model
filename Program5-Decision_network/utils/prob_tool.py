...
class factor:
    ...
    def show_dist( self):
        ...
        for i in range( 1, ndim+1):
            ...
            col.append(fname)
        return pd.DataFrame.from_dict(self.parconfig_dist, 
                                     orient='index', 
                                     columns = col)
...
def fac_div(x, y):
    ...
    res_distribution =   x_distribution[tuple([slice(None)]*len(x.get_variables())+[None]*len(y_not_in_x))] \
                       / y_distribution[tuple([None]*len(x_not_in_y)+[slice(None)])]
    res_distribution[np.isnan(res_distribution)] = 0.
    return factor(list(x_not_in_y)+list(xy)+list(y_not_in_x), res_distribution)
...
def fac_plus(x, y):
    ...
    U(x, y) = Ui(x)+ Uj(x) 
    '''
    if (x.get_variables() ==  y.get_variables()).all()==False:
        raise Exception('One of the factors is None')
    variables = x.get_variables()
    distribution = x.get_distribution() + y.get_distribution()
    return factor( variables, distribution)
...
def fac_max(x, variables):
    ...
    res_distribution = np.max(x.get_distribution(),
                              tuple(np.where(np.isin(x.get_variables(), variables))[0]))
    max_act             = np.argmax(x.get_distribution(),
                              np.where(np.isin(x.get_variables(), variables))[0][0])
        
    return factor(res_variables, res_distribution), max_act
